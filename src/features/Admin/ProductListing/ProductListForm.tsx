...
export default function ProductListForm(props: PropTypes) {
	...
	const [IsHelmetCategorySelected, setIsHelmetCategorySelected] = useState<boolean>(false);

	const categoryChangeHandler = (value: any) => {
		if (value === 'HELMET') {
			setIsHelmetCategorySelected(true);
		} else {
			setIsHelmetCategorySelected(false);
		}
	};
		...
		<Space direction="vertical" size="large">
			<Form
				...
				initialValues={{
					...
					helmetType: listProduct.productInformation.helmetType,
			...
			>
				<Row gutter={[32, 0]}>
					...
					{IsHelmetCategorySelected && (
						<Col md={12}>
							<Form.Item name="helmetType" label="Helmet Type" rules={[{ required: true, message: 'Required' }]}>
								<Select
									showSearch
									placeholder="Select a helmet type"
									optionFilterProp="children"
									filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
									options={[
										{ value: 'FULL_FACE', label: 'Full Face' },
										{ value: 'HALF_FACE', label: 'Half Face' },
										{ value: 'FLIP_UP', label: 'Flip Up' },
